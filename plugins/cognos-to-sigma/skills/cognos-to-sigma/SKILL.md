...
- **Source visual reference — ASK THE USER for a screenshot up front (Cognos has NO render API).** Unlike Tableau/Power BI, there is no `/api/v1` or `/bi/v1` endpoint that renders a dashboard/exploration or report to an image — no export / thumbnail / report-output path exists on the surface. So, mirroring `powerbi-to-sigma`, **request a screenshot of each dashboard/report being migrated at the start** (one per Cognos tab) and keep it as the **visual parity target**. Phase 4 renders the *Sigma* side (`apply-layout.mjs` → `sigma-export-png.py` full-page PNG); parity = compare the user's source shot against that PNG. Without the source shot you have only a data check, not a visual one — say so rather than skip it.
