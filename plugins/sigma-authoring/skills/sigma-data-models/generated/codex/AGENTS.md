...
| `reference/workflows/migrate-csv-org.md` | Move a CSV-backed data model between Sigma organizations without re-uploading the CSV. Discovers the hidden Databricks upload table through element-query SQL, rewrites it as a destination custom SQL source, and creates only behind explicit mutation gates. |
