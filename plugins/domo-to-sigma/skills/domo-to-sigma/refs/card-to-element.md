...
3. **Analyzer Quick Filters** (`slicers[]` / `quickFilters[]`) → card-scoped
   controls. Charts/tables/pivots use a hidden table source for picker values
   and filtering; they are not permanent `main.filters`.
...
(`["-3"]` → `[-3]`), booleans become JSON booleans, and numeric-looking values
