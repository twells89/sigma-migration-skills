...
module AnchorVerify
  ...
  def verify(anchors, exports, extract_tol: nil, truncated: nil, target_scopes: nil)
    ...
    target_for = lambda do |a, order|
      ...
      exact = el_names.select { |name| name.to_s.strip.casecmp?(hint) }
      exact.empty? ?
        { 'names' => [], 'via' => 'unresolved',
          'error' => 'hint matched no exact element name and no provenance scope was supplied' } :
        { 'names' => exact, 'via' => 'exact-element-name' }
    end
