...
class AnchorValueTest(unittest.TestCase):
    ...
    def test_provenance_target_blocks_common_token_cross_tile_match(self):
        anchors = [
            {
                "id": "a1",
                "raw": "312",
        ...
        tiles = verify_anchors.normalize_actuals(
            {
                "tiles": [
                    {
                        "id": "el-overall",
                        "name": "1. OVERALL CS TICKET SUMMARY",
                        "rows": [["01-Jun-2026", 312]],
                    },
                    {
                        "id": "el-client",
                        "name": "6. CS TICKET SUMMARY BY CLIENT",
                        "rows": [["Example Co", 4]],
                    },
                ]
            }
        )
        scopes = verify_anchors.resolve_target_scopes(
            anchors,
            tiles,
            {
                "el-overall": {
                    "worksheet": "OVERALL CS TICKET SUMMARY - CS"
                },
                "el-client": {
                    "worksheet": "CS TICKET SUMMARY BY CLIENTS- CS"
                },
            },
        )
        verdict = verify_anchors.verify(anchors, tiles, target_scopes=scopes)

        self.assertFalse(verdict["pass"])
        self.assertEqual(
            "6. CS TICKET SUMMARY BY CLIENT",
            verdict["missing"][0]["best_candidate"]["element"],
        )
        self.assertEqual(
            ["6. CS TICKET SUMMARY BY CLIENT"], scopes["a1"]["names"]
        )

    def test_unresolved_hint_fails_closed(self):
        anchors = [
            {
                "id": "a1",
                "raw": "104",
                "label": "Stores",
                "sigma_element_hint": "Missing Worksheet",
            }
        ]
        tiles = verify_anchors.normalize_actuals({"Unrelated Tile": [[104]]})
        verdict = verify_anchors.verify(
            anchors,
            tiles,
            target_scopes={
                "a1": {
                    "names": [],
                    "via": "unresolved",
                    "error": "hint matched no exact tile or provenance worksheet",
                }
            },
        )
        self.assertFalse(verdict["pass"])
        self.assertEqual("unresolved", verdict["missing"][0]["target_resolution"])

    def test_missed_hint_does_not_claim_tile_coverage(self):
        anchors = [
            {
                "id": "a1",
                "raw": "999",
        ...
        tiles = verify_anchors.normalize_actuals(
            {
                "tiles": [
                    {"id": "gamma", "name": "Gamma Chart", "rows": [[100]]},
                    {"id": "other", "name": "Other Chart", "rows": [[999]]},
                ]
            }
        )
        verdict = verify_anchors.verify(anchors, tiles)

        self.assertFalse(verdict["pass"])
        self.assertEqual(0, verdict["anchor_coverage"]["covered"])
        self.assertEqual(
            ["Gamma Chart", "Other Chart"],
            verdict["anchor_coverage"]["uncovered"],
        )
