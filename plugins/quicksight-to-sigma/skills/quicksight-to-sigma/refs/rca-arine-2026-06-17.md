...
**RESOLVED 2026-07-10 (quicksight-theme-color-fidelity):** `quicksight-discover.py` now resolves the theme palette (`resolve_theme`): ThemeArn (dashboard-definition) or `describe-analysis` (analysis) → account default → CLASSIC; custom account themes fetched via `describe-theme`, AWS-managed built-ins served from a carried palette table (built-ins share the CLASSIC data palette, verified against a rendered dashboard). It writes `signals.json.theme` = `{name, dataColors, primaryColor, emptyFill, isDark}`. `build-workbook-from-quicksight.rb` applies it as top-level `themeOverrides.categoricalScheme` (the only spec path to pie/donut slice colors + the palette for all categorical charts) and `color:{by:single}` on single-measure bar/line/area (QS paints those in the theme primary). `themeName:Dark` when the QS theme is dark. **Known limitation:** `categoricalScheme` is a single *global, positional* palette applied in Sigma's category sort order, so a pie's slice→label color assignment can differ from QuickSight's (which assigns by value-descending) even though the color SET matches; per-slice label parity would need per-element control Sigma's global scheme doesn't offer. Inline chrome hexes (title color etc.) still not mapped.
