...
**Color fidelity (theme palette).** QuickSight visual colors are theme-driven and NOT in the definition export, so the builder reads `signals.json.theme` (resolved in Phase 2) and applies it: top-level `themeOverrides.categoricalScheme` = the QS data-color palette (the ONLY spec path to pie/donut slice colors, and the palette for every categorical chart), plus `color:{by:single, value:<primaryColor>}` on single-measure bar/line/area (QuickSight paints those in the theme's primary data color), and `themeName:Dark` when the QS theme is dark. No `theme` in signals (e.g. offline `--from-fixtures`) → colors fall to Sigma defaults, unchanged. **Limitation:** `categoricalScheme` is one *global, positional* palette applied in Sigma's category sort order — the color SET matches QuickSight but a pie's slice→label assignment can differ (QS assigns by value-descending); per-slice label parity isn't expressible via Sigma's single global scheme.
